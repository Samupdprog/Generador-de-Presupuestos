...
export function createQuoteWorkflowRepository(db: Database) {
  return {
    ...
    async deleteLine(input: { installationId: string; quoteId: string; expectedRevision: number; lineId: string }) {
      return db.transaction(async (tx) => { await tx.delete(quoteLineDiscounts).where(eq(quoteLineDiscounts.quoteLineId, input.lineId)); await tx.delete(quoteLineLaborEntries).where(eq(quoteLineLaborEntries.quoteLineId, input.lineId)); await tx.delete(quoteLines).where(and(eq(quoteLines.id, input.lineId), eq(quoteLines.quoteId, input.quoteId))); return finalize(tx, input.quoteId, input.installationId, input.expectedRevision, "quote.line.deleted"); });
    },
